...
public class Gauss {
    ...
    public static String[] parametrikGauss(double[][] m) {
        ...
        char[] parameter = { 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r',
                's', 't', 'u', 'v', 'w', 'x', 'y', 'z' };
        ...
        int effRow = MatrixOP.getRowEff(OBEdmatrix) - MatrixOP.CountRowZero(OBEdmatrix); // jumlah row yang tidak full
                                                                                         // zero

        // STEP 1 : Looping untuk cari dulu solusi yang bisa diambil dari baris yang
        // hanya ada leading one dan result
            ...
            for (int j = nCol - 1; j > MatrixOP.getIdxColElNotZero(OBEdmatrix, i); j--) {
                ...
                if (j == nCol - 1) {
                    ...
                    if (OBEdmatrix[i][j] > 0) {
                        if (OBEdmatrix[i][j] == 1) {
                            arrayfinal[MatrixOP.getIdxColElNotZero(OBEdmatrix, i)] += " - "
                                    + String.valueOf(parameter[j]);
                        } else {
                            arrayfinal[MatrixOP.getIdxColElNotZero(OBEdmatrix, i)] += " - "
                                    + String.valueOf(OBEdmatrix[i][j]) + String.valueOf(parameter[j]);
                        }
                    } else if (OBEdmatrix[i][j] < 0) {
                        if (arrayfinal[MatrixOP.getIdxColElNotZero(OBEdmatrix, i)] == "") {
                            arrayfinal[MatrixOP.getIdxColElNotZero(OBEdmatrix, i)] += String
                                    .valueOf(Math.abs(OBEdmatrix[i][j])) + String.valueOf(parameter[j]);
                        } else {
                            if (OBEdmatrix[i][j] == -1) {
                                arrayfinal[MatrixOP.getIdxColElNotZero(OBEdmatrix, i)] += " + "
                                        + String.valueOf(parameter[j]);
                                ...
                                arrayfinal[MatrixOP.getIdxColElNotZero(OBEdmatrix, i)] += " + "
                                        + String.valueOf(Math.abs(OBEdmatrix[i][j])) + String.valueOf(parameter[j]);
                            }
                        }
                    } else if (OBEdmatrix[i][j] == 0) {
                        arrayfinal[MatrixOP.getIdxColElNotZero(OBEdmatrix, i)] += "";
    ...
    public static String[] arrayResultUniqueSol(double[][] m) {
        ...
        for (int i = 0; i < MatrixOP.getColEff(m) - 1; i++) {
            strarrayhasil[i] = String.valueOf(arrayhasil[i]);
        }
